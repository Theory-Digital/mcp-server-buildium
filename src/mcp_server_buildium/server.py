...
def create_server(config: BuildiumConfig | None = None) -> FastMCP:
    """Create a configured MCP server."""
    resolved_config = config or BuildiumConfig.from_env()
    buildium_client = BuildiumClient(config=resolved_config)
    mcp = FastMCP("buildium")

    # Register tools based on enabled categories
    if resolved_config.is_category_enabled("associations"):
        register_association_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("leases"):
        register_lease_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("rentals"):
        register_rental_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("applicants"):
        register_applicant_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("tenants"):
        register_tenant_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("owners"):
        register_owner_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("units"):
        register_unit_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("vendors"):
        register_vendor_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("tasks"):
        register_task_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("bills"):
        register_bill_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("files"):
        register_file_tools(mcp, buildium_client)
    if resolved_config.is_category_enabled("bank_accounts"):
